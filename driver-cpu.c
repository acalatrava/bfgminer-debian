...
extern bool scanhash_shani(struct thr_info *, struct work *, uint32_t max_nonce, uint32_t *last_nonce, uint32_t nonce);
extern bool scanhash_arm_crypto(struct thr_info *, struct work *, uint32_t max_nonce, uint32_t *last_nonce, uint32_t nonce);
extern bool sha_ni_available(void);
extern bool arm_crypto_available(void);
...
#endif
#ifdef WANT_X8664_SHANI
	[ALGO_SHANI] = (sha256_func)scanhash_shani,
#endif
#ifdef WANT_ARM64_CRYPTO
	[ALGO_ARM_CRYPTO] = (sha256_func)scanhash_arm_crypto,
#endif
};
#endif
...
#if defined(WANT_X8664_SHANI)
	if (sha_ni_available())
	{
		bench_algo(&best_rate, &best_algo, ALGO_SHANI);
	}
#endif

#if defined(WANT_ARM64_CRYPTO)
	if (arm_crypto_available())
	{
		bench_algo(&best_rate, &best_algo, ALGO_ARM_CRYPTO);
	}
#endif
...
static float cpu_min_nonce_diff(struct cgpu_info *const proc, const struct mining_algorithm *const malgo)
{
	return minimum_pdiff;
}

static bool scanhash_generic(struct thr_info *const thr, struct work *const work, const uint32_t max_nonce, uint32_t *const last_nonce, uint32_t n)
{
